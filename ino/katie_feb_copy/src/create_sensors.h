...
void charger() {
  ...
  // When Katie moves onto dock switch to safe mode... We need different loop for plug
  if ((CHARGING_SOURCE_STATE > 0) && (OI_MODE_STATE == OI_SAFE)) 
    delay(100); // gives time for create to set chargin source state
    // xCOIChangeMode(CMD_MODE_PASSIVE);
