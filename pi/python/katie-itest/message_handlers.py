...
from websocket_client import ControlMessage
from other_threads import AutoTakeThread, ActionGreetThread, ActionShooThread
from protocol import Protocols
from action_threads_holder import ActionThreads
...
class HandlerForActionFromSerial():
    ...
    def process(self, message):
        ...
        if "#dis" in message:
            self.ws_client.send(
                        ujson.dumps(
                            ControlMessage("topic.current.heading", ujson.dumps([self.robot_id, message]))))

        if "#ana" in message:
            self.ws_client.send(
                        ujson.dumps(
                            ControlMessage("topic.rear.distance", ujson.dumps([self.robot_id, message]))))
