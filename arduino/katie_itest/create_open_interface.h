//////////////////////////////////////////////////////////////////
//Arduino-iRobot-Create-API
//Author: Michael Dillon
//link:https://github.com/michaelcdillon/Arduino-iRobot-Create-API
//#include <SoftwareSerial.h>
//#include <Arduino.h>

// Hold down powerbutton on create 2 until decending pitches
#define COI_BAUDRATE            19200
...
// Global varss
SoftwareSerial coiSerial = SoftwareSerial(7,4);
...
/**
  ...
  * send it commands.
  */
void COIInitialize (int rxPin, int txPin) {
    delay (IROBOT_CREATE_BOOT_TIME);
    COISerialSetup (rxPin, txPin);
...
void reverseScript() {
  COISerialWriteByte(CMD_SCRIPT);
  COISerialWriteByte(13); /// script length
  ...
  // wait until we have driven 6.5 cm
  int distance = -65; // in mm
  COISerialWriteByte(156);
  COISerialWriteByte(highByte(distance));
  COISerialWriteByte(lowByte(distance));
  
  // stop
  COISerialWriteByte (137);
  COISerialWriteByte (0);
  COISerialWriteByte (0);
  COISerialWriteByte (0);
  COISerialWriteByte (0);
  ...
  delay(100);
  //COIPlayScript();
  COISerialWriteByte(CMD_PLAY_SCRIPT);
}


void forwardScript(int distance) {
  COISerialWriteByte(CMD_SCRIPT);
  COISerialWriteByte(13); /// script length
...
int default_turn_velocity = 200;
int turn_velocity = default_turn_velocity;
int true_heading = 0; //robot always starts on 0 until we get compass.


void turnDegreePIN(int degree) {
 
  COISerialWriteByte(CMD_SCRIPT);
  COISerialWriteByte(17); /// script length
  
  COISetDigitalOutput(2,HIGH);
  ...
  // wait until we have driven 10 cm
  COISerialWriteByte(CMD_WAIT_ANGLE);
  COISerialWriteByte(highByte(degree));
  COISerialWriteByte(lowByte(degree));
  ...
  true_heading = true_heading + degree;
  if (true_heading > 360) {
    true_heading = true_heading - 360;
  } else if (true_heading < 0) {
...
void turnDegreeVelocityPIN(int degree, int velocity) {
  turn_velocity = velocity;
  turnDegreePIN(degree);
  turn_velocity = default_turn_velocity;
}
  
void turnDegreeVelocity(int degree, int velocity) {
  turn_velocity = velocity;
  turnDegree(degree);
  turn_velocity = default_turn_velocity;
}
...
int COIRecieveSingleSensor () {
    ...
    // flush buffer, there should be nothing in there.
    while (COISerialAvailable() ) {
      COISerialReadByte();
      // check using light or beep if test proves inconclusive...
    }
...
/**
  * Wait Angle
  */  
void COIWaitAngle(int angle){
  COISerialWriteByte(CMD_WAIT_ANGLE);
  COISerialWriteByte(highByte(angle));
  COISerialWriteByte(lowByte (angle)); 
}

/**
  * Wait Distance
  */  
void COIWaitDist(int distance){
  COISerialWriteByte(CMD_WAIT_DIST);
  COISerialWriteByte(highByte(distance));
  COISerialWriteByte(lowByte (distance)); 
}
...
#define BUMP_EVENT       5
#define LEFT_BUMP_EVENT  6
#define RIGHT_BUMP_EVENT 7


/**
  * Wait Event
  */  
void COIWaitEvent(int event){
  COISerialWriteByte(CMD_WAIT_EVENT);
  COISerialWriteByte(event);
}
 
/**
  * Run the demo functions
  */  
#define DEMO_COMMAND       0x88
#define ABORT_CURRENT_DEMO   -1
#define COVER                 0
#define COVER_AND_DOCK        1
#define SPOT_COVER            2
#define MOUSE                 3
#define DRIVE_FIGURE_EIGHT    4
#define WIMP                  5
#define HOME                  6
#define TAG                   7
#define PACHELBEL             8
...
void COIDemo(int code){
  if(!DEMO_RUNNING || code==ABORT_CURRENT_DEMO) {
      COISerialWriteByte(DEMO_COMMAND);
      COISerialWriteByte(code);
      if(code!=ABORT_CURRENT_DEMO) DEMO_RUNNING = true;
      else DEMO_RUNNING = false;
  }
  else return;
}
